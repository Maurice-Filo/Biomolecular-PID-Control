...
N = 5;                              % Number of grid points for the swept parameter (In paper: N = 25)
Iterations = 1;                  	% Number of Outer Iterations for the batch stochastic Simulations: Each Iteration runs N_Trajectories using parfor (In paper: Iterations = 250)
...
%% Construct Parameters From Desired Gains
Gains.K_P  = 0; Gains.K_I = 0.03; Gains.K_D = 0; Gains.omega_c = 0.5;
...
K_D_Max = (u_bar/r) * (u_bar/Parameters.mu/Gains.K_I/4 - 1/Gains.omega_c);
K_D_vector = linspace(0, K_D_Max-0.05, N);
...
clear ans MeanY_Iteration VarY_Iteration iter FinalDistribution_Iteration

RunTime = toc(RunTime);
save APIDF2_Deg_Star_Variance_Sweep

else
load APIDF2_Deg_Star_Variance_Sweep;
...
delta_vector = zeros(N_Refined,1);
beta_vector = zeros(N_Refined,1);
eta_vector = zeros(N_Refined,1);
for i = 1 : N_Refined
    Gains.K_D = K_D_vector_Refined(i);
    [Parameters, Feasible] = Gains2Params(Gains, Parameters, r, SupportingInput);
    k_vector(i) = Parameters.k;
    delta_vector(i) = Parameters.delta;
    beta_vector(i) = Parameters.beta;
    eta_vector(i) = Parameters.eta;
end
...
%% Set Figure 1
Figure1_Name = 'APIDF2_Deg_Star_BioParameters';
...
%% Set Figure 2
Figure2_Name = 'APIDF2_Deg_Star_StationaryVariance';
...
%% Set Figure 3
Figure3_Name = 'APIDF2_Deg_Star_StochasticSimulations';
...
Handle_Axis1 = axes(Handle_Figure1);
    Handle_Axis1.Position = [0.175, 0.1, 0.58, 0.82];
    Handle_Axis1.Box = 'on';
    ...
    Handle_Axis1.XLabel.String = '$\delta$';
    Handle_Axis1.YLabel.String = '$k$';
    Handle_Axis1.ZLabel.String = '$\beta$';
    Handle_Axis1.XLabel.Interpreter = 'latex';
    Handle_Axis1.YLabel.Interpreter = 'latex';
    Handle_Axis1.ZLabel.Interpreter = 'latex';
    view(Handle_Axis1, [40, 40])
    colormap(Handle_Axis1, 'turbo');
        ...
        Handle_ColorBar.Position(4) = 0.7;
        Handle_ColorBar.Label.String = '$K_D$';
        Handle_ColorBar.Label.Interpreter = 'latex';
scatter3(Handle_Axis1, delta_vector, k_vector, beta_vector, MarkerSize*3, linspace(K_D_vector_Refined(1), K_D_vector_Refined(N_Refined), N_Refined), 'filled');
plot3(Handle_Axis1, Parameters_Nominal_1.delta, Parameters_Nominal_1.k, Parameters_Nominal_1.beta, 'Marker', 'o', 'MarkerSize', MarkerSize, 'Color', 'k', 'LineStyle', 'none', 'MarkerFaceColor', DColor1);
plot3(Handle_Axis1, Parameters_Nominal_2.delta, Parameters_Nominal_2.k, Parameters_Nominal_2.beta, 'Marker', 'o', 'MarkerSize', MarkerSize, 'Color', 'k', 'LineStyle', 'none', 'MarkerFaceColor', DColor2);
plot3(Handle_Axis1, Parameters_Nominal_3.delta, Parameters_Nominal_3.k, Parameters_Nominal_3.beta, 'Marker', 'o', 'MarkerSize', MarkerSize, 'Color', 'k', 'LineStyle', 'none', 'MarkerFaceColor', DColor3);
plot3(Handle_Axis1, Parameters_Nominal_4.delta, Parameters_Nominal_4.k, Parameters_Nominal_4.beta, 'Marker', 'o', 'MarkerSize', MarkerSize, 'Color', 'k', 'LineStyle', 'none', 'MarkerFaceColor', DColor4);

Handle_Annotation = annotation(Handle_Figure1, 'textbox', [0.79, 0.79, 0.1, 0.1]);
    Handle_Annotation.String = {'Hard Upper'; 'Limit'};
    Handle_Annotation.EdgeColor = 'none';
    Handle_Annotation.HorizontalAlignment = 'center';
    Handle_Annotation.VerticalAlignment = 'top';
    Handle_Annotation.FontSize = FontSize_Small;
...
Handle_Axis2 = axes(Handle_Figure2);
    ...
    Handle_Axis2.XLabel.String = '$K_D \approx \frac{\delta}{\eta(\bar z_1 + \bar z_2)}$';
    Handle_Axis2.XLabel.Position(2) = 96.8;
...
StationaryVariance_Smoothed = smoothdata(StationaryVariance, 'loess');
...
Handle_Histogram_3 = histogram(Handle_Axis32, FinalDistribution_3, 'BinWidth', 1, 'FaceColor', DColor3, 'Orientation', 'Horizontal', 'Normalization', 'probability');
